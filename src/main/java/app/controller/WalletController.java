 ...
 *
 * The frontend stores the userId in localStorage after login.
 * When the user sends money, the frontend sends that userId.
 *
 * ATTACK SCENARIO:
 *   1. Victim (userId=1) logs in → frontend stores userId=1 in localStorage
 *   2. Attacker logs in  → frontend stores userId=2 in localStorage
 *   3. Attacker opens DevTools → changes localStorage userId from 2 to 1
 *   4. Attacker sends money using userId=1 → steals victim's money!
 *
 * WHY IT WORKS:
 *   The server blindly trusts the userId from the client.
 *   It never checks: "Is this userId actually the logged-in user?"
 *
...
public class WalletController {
     ...
     *
     * ⚠️ ATTACK: Change userId in localStorage to someone else's ID,
     *    then send money. The server will transfer FROM that user's wallet!
     */
    @PostMapping("/send")
